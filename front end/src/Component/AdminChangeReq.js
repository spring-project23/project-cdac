  ...
  const updateRequest = () => {
    ...
    MechanicService.getOneMechanic(mechanicId).then((response) => {
      ...
      RequestService.getOneRequest(updatedrequestId.requestId).then((res) => {
        ...
        axios.put(`http://localhost:9000/Request-api/${updatedrequestId.requestId}`, newRequest, {
          headers: {
      ...
      <div className="row" style={{ margin: "50px 300px" }}>
        ...
        <Container className="col">
          ...
          <Form.Group className="mb-3" controlId="formBasicEmail">
            ...
            <Form.Control type="number" placeholder="Repair Cost"
              onChange={(e) => { setCost(e.target.value) }} />
          </Form.Group>
          <Form.Group className="mb-3">
            <Form.Label>Assign Mechanic</Form.Label>
            <Form.Select onChange={(e) => { setMechanicId(e.target.value) }}>
              <option>Selected</option>
              {mechanics.map(element => {
                return <option value={element.mechanicID} key={element.mechanicID} >{element.firstName}</option>
              })}
            </Form.Select>
          </Form.Group>
          <Form.Group className="mb-3">
