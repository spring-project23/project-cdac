...
class CustomerDashBoard extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      requests: [],
      pendingCount: 0, approvedCount: 0, releasedCount: 0,
      totalBill: 0
    };
  }
  ...
  componentDidMount() {
    ...
    CustomerService.getCustomerByUser(user).then((res) => {
      ...
      RequestService.getAllRequestByCustomer(res.data.customerID)
        .then((res) => {
          var pcnt = 0, acnt = 0, rcnt = 0, totalCost = 0;
          console.log(res.data)
          res.data.forEach(element => {
            if (element.status === "Pending")
              pcnt++;
            else if (element.status === "Approved" || element.status === "Repairing" || element.status === "Repairing Done") {
              acnt++;
          ...
          this.setState({ requests: res.data, pendingCount: pcnt, approvedCount: acnt, releasedCount: rcnt, totalBill: totalCost })
            ...
            >
              <Card.Body>
                ...
                <div className="text-center pt-3">
                  <h5><b>New Request Made</b></h5></div>
            ...
            >
              <Card.Body>
                ...
                <div className="text-center pt-1">
                  <FiRotateCw /> </div>
                <div className="text-center pt-3">
                  <h5><b>Vehicle Repair In Progress</b></h5>
                </div>
              </Card.Body>
            </Card>
          </div>
